...
public final class RatTerm {
  ...
  public RatTerm add(RatTerm arg) {
	  ...
	  return new RatTerm(arg.coeff.add(this.coeff),expt);
  }
  ...
  public RatTerm div(RatTerm arg) {
	  if(arg.isNaN() || arg.isZero() || this.isNaN()){
	    	return NaN;
	    }
	  RatTerm result = new RatTerm(this.coeff.div(arg.coeff), this.expt-arg.expt);
	  return result;
  }
  ...
  public RatTerm differentiate() {
   ...
   RatNum exptCopy = new RatNum(expt);
   RatTerm result = new RatTerm(coeff.mul(exptCopy), expt-1);
   return result;
  }
  ...
  public RatTerm antiDifferentiate() {
    if(isNaN())
    	return NaN;
    RatNum exptCopy = new RatNum(expt+1);
    RatTerm result = new RatTerm(coeff.div(exptCopy), expt+1);
    return result;
  }
